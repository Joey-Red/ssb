...
describe('Pro Lab final conservative VOD recovery batch', () => {
  ...
  it('preserves all 800 records while reducing unresolved source indexes from 120 to 107', () => {
    expect(proVodCatalog).toHaveLength(800)
    expect(new Set(proVodCatalog.map((vod) => vod.id)).size).toBe(800)
    expect(proVodCatalog.filter((vod) => vod.linkKind === 'source-index')).toHaveLength(107)
    expect(proVodCatalog.filter((vod) => vod.linkKind !== 'source-index')).toHaveLength(693)
  })
