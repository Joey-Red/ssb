    ...
    <div className="app-shell">
      ...
      <aside className="sidebar">
        <a className="brand" href={hrefFor('/')}>
          ...
          <span><strong>SSBU</strong><small>TRAINING GUIDE</small></span>
        </a>
        <p className="nav-label">Workspace</p>
        <nav className="side-nav" aria-label="Primary">
          <a className={route.page === 'roster' ? 'is-active' : ''} href={hrefFor('/')}>
            <span aria-hidden="true">▦</span><strong>Roster</strong><em>{roster.length}</em>
          </a>
          <a className={route.page === 'fighter' ? 'is-active' : ''} href={hrefFor('/fighter/mario')}>
            <span aria-hidden="true">◎</span><strong>Fighter guides</strong><em>{allGuides.length}</em>
          </a>
          <a className={route.page === 'practice' ? 'is-active' : ''} href={hrefFor('/practice/mario')}>
            <span aria-hidden="true">▶</span><strong>Practice</strong>
          </a>
          <a className={route.page === 'about' ? 'is-active' : ''} href={hrefFor('/about')}>
            <span aria-hidden="true">?</span><strong>How to read it</strong>
          </a>
        </nav>
        <div className="sidebar-footer">
          <span className="health-dot" aria-hidden="true"/><strong>Static & source-aware</strong>
          <p>No server, login, telemetry, or runtime data dependency.</p>
        </div>
      </aside>
      <div className="app-column">
        <header className="topbar">
          ...
          <div className="topbar-status">
            ...
            <strong>Ready</strong><span>{allGuides.length}/{roster.length} guides</span>
          </div>
        </header>
