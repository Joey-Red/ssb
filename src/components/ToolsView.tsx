...
import { discoverFastMoves, type DiscoveryPool } from '../lib/toolDiscovery'
...
export function ToolsView() {
  ...
  const frameDataByFighterId = frameState.data?.byFighterId
  const frameMoveCount = frameState.data?.moveCount ?? 0
  ...
  const [leftMoveId, setLeftMoveId] = useState('')
  const [rightMoveId, setRightMoveId] = useState('')
  const [search, setSearch] = useState('')
  const [maxStartup, setMaxStartup] = useState(6)
  const [pool, setPool] = useState<DiscoveryPool>('attacks')
  ...
  const discovered = useMemo(() => frameState.data ? discoverFastMoves([...frameState.data.byFighterId.values()], {
    maxStartup,
    query: search,
    pool,
    limit: 80,
  }) : [], [frameState.data, maxStartup, pool, search])

  const dataNotice = frameState.loading ? (
    <section className="panel pending-panel" aria-live="polite"><span className="pending-icon" aria-hidden="true">…</span><div><p className="eyebrow">Static frame snapshot</p><h2>Loading frame tools</h2><p>The full frame dataset is fetched only when this workspace is opened.</p></div></section>
  ) : frameState.error ? (
    <section className="panel pending-panel"><span className="pending-icon" aria-hidden="true">!</span><div><p className="eyebrow">Static frame snapshot</p><h2>Frame tools unavailable</h2><p>{frameState.error}</p></div></section>
  ) : null

  return <div className="page-stack tools-view">
    <section className="hero-panel tools-hero"><div><p className="eyebrow">Frame-data workspace</p><h1>Compare. Discover. Verify.</h1><p className="hero-copy">Cross-roster move comparison and out-of-shield startup references over {frameState.loading ? 'the full' : frameMoveCount.toLocaleString()} committed move rows. Raw source notation stays visible.</p></div><div className="hero-stats"><div><strong>{frameDataByFighterId?.size ?? roster.length}</strong><span>fighters</span></div><div><strong>{frameState.loading ? '…' : frameMoveCount.toLocaleString()}</strong><span>move rows</span></div></div></section>
    ...
    {frameState.data && <>
      ...
      <section className="panel" aria-labelledby="discover-title"><div className="section-heading"><div><p className="eyebrow">Move discovery</p><h2 id="discover-title">Find fast buttons across the roster</h2></div></div><div className="discovery-controls"><label><span>Search</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="fighter, move, category…"/></label><label><span>Move pool</span><select value={pool} onChange={(event) => setPool(event.target.value as DiscoveryPool)}><option value="attacks">Attacks only</option><option value="all">All sourced rows</option></select></label><label><span>Maximum startup: {maxStartup}f</span><input type="range" min="1" max="30" value={maxStartup} onChange={(event) => setMaxStartup(Number(event.target.value))}/></label></div><p className="discovery-note">Attacks only excludes pummels, throws/other grab rows, defensive actions, and miscellaneous effects so the default list behaves like an actual fast-button finder.</p><div className="discovery-list">{discovered.map(({ fighter, move }) => <a href={`#/fighter/${roster.find((entry) => entry.id === fighter.fighterId)?.slug ?? fighter.fighterId}`} key={`${fighter.fighterId}-${move.id}`}><span><strong>{fighter.name}</strong><small>{move.name} · {move.category}</small></span><b>{move.startup}</b></a>)}</div>{discovered.length === 0 && <div className="empty-state"><h3>No move matches</h3><p>Raise the startup cap, change the move pool, or change the search.</p></div>}</section>
    </>}
  </div>
}
