        ...
        <aside className="fighter-side" aria-label={`${fighter.name} quick guide`}>
          <section className="panel sticky-panel">
            ...
            <ol className="quick-list">
              {guide.quickGuide.map((item, itemIndex) => (
                <li key={item}>
...
function FighterHeader({ name, series, archetype, memoryAid }: { name: string; series: string; archetype: string; memoryAid: string }) {
  ...
  return (
    <section className="fighter-hero">
      <div className="fighter-hero__mark" aria-hidden="true">{initials}</div>
      <div className="fighter-hero__content">
        <div className="fighter-hero__meta"><span>{series}</span><span>•</span><span>{archetype}</span></div>
        <h1>{name}</h1>
        <div className="memory-aid"><span>Memory aid</span><p>{memoryAid}</p></div>
      </div>
    </section>
  )
}
...
function FighterPager({ previous, next }: { previous: (typeof roster)[number] | undefined; next: (typeof roster)[number] | undefined }) {
  ...
  return (
    <nav className="fighter-pager" aria-label="Adjacent fighters">
      <a href={hrefFor(`/fighter/${previous.slug}`)}><span>← Previous</span><strong>{previous.name}</strong></a>
      <a href={hrefFor(`/fighter/${next.slug}`)}><span>Next →</span><strong>{next.name}</strong></a>
    </nav>
  )
}
...
function NotFoundFighter() {
  return (
    <section className="panel empty-state">
      ...
      <p>The roster route does not match a known fighter.</p>
